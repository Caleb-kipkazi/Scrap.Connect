...
export default function AdminLogin({ navigation }) {
    // Changed state variable name from centerName to email for clarity and consistency
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    const [isPasswordVisible, setPasswordVisible] = useState(false);
    const [loading, setLoading] = useState(false);
    ...
    const handleLogin = async () => {
        if (!email || !password) {
            Alert.alert("Error", "Center Email and Password are required."); // Changed alert message
            return;
        }
        ...
        try {
            ...
            // Destructure the response data according to what your backend centerSignin sends:
            // { token, centerId, centerName, email }
            const { token, centerId, centerName: fetchedCenterName } = response.data; 

            await AsyncStorage.setItem("adminToken", token); // Store token with key 'adminToken'
            await AsyncStorage.setItem("centerId", centerId); // Store center's _id
            await AsyncStorage.setItem("centerName", fetchedCenterName); // Store center's name, crucial for Assign.js

            // Updated success message to use fetchedCenterName
            Alert.alert("Login Successful", `Welcome back, Admin ${fetchedCenterName}!`);
            navigation.navigate("AmainNavigator"); // Navigate to admin main navigator
        } catch (error) {
            console.error("Admin Login Error:", error);
            const message =
    ...
    return (
        <View style={styles.container}>
            <View style={styles.loginBox}>
                ...
                <TextInput
                    placeholder="Center Email" // Changed placeholder to 'Center Email'
                    placeholderTextColor="#ccc"
                    style={styles.input}
                    keyboardType="email-address" // Changed keyboard type to email
                    autoCapitalize="none"
                    value={email} // Using 'email' state
                    onChangeText={setEmail} // Using 'setEmail' setter
                />
